<template>
  <div :class="[done ? 'task_done' : '', 'task']">
    <div class="task__info">
      <label class="task__checkbox-label">
        <input
          @change="taskIsDone(task.id, done)"
          type="checkbox"
          name="done"
          class="task__done-check"
          :checked="checked"
        />
        <span class="task__custom-checkbox"></span>
      </label>
      <div class="task__text">
        <h2 class="task__title">{{ task.text }}</h2>
    ...
    <div class="task__controls">
      <button
        ...
        type="button"
        class="button button_delete-task"
      >
        <svg
          stroke="currentColor"
          fill="currentColor"
...
export default {
  ...
  methods: {
    removeTask (id, done) {
      this.$emit ('removeTask', id, done);
    },
    taskIsDone (id, done) {
      this.$emit("taskIsDone", id, done);
    },
    editTask (id) {
